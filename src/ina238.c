 ...
 */
static int ina238_probe(ina238_device_t *dev) {
   i2c_device_t i2c_dev = { .fd = dev->fd, .address = dev->i2c_addr, .bus = NULL };
   ...
   if (value != INA238_MFG_ID_TI) {
      OLOG_ERROR("Invalid manufacturer ID: 0x%04X (expected 0x%04X)", value, INA238_MFG_ID_TI);
      return -1;
   }
   ...
   if (device_id != INA238_MFG_DIE) {
      OLOG_ERROR("Invalid device ID: 0x%04X (expected 0x%04X)", device_id, INA238_MFG_DIE);
      return -1;
   }
 ...
 */
int ina238_init(ina238_device_t *dev,
                const char *i2c_bus,
                uint8_t i2c_addr,
                float r_shunt,
                float max_current) {
   ...
   dev->range = (dev->max_current > (DEFAULT_MAX_CURRENT - 1.0f)) ? INA238_ADCRANGE_HIGH
                                                                  : INA238_ADCRANGE_LOW;
   ...
   if (i2c_open_device(&i2c_dev, i2c_bus, i2c_addr) < 0) {
      OLOG_ERROR("Failed to open I2C device %s at address 0x%02X", i2c_bus, i2c_addr);
      return -1;
   }
...
int ina238_read_measurements(ina238_device_t *dev, ina238_measurements_t *measurements) {
   ...
   measurements->valid = (measurements->bus_voltage != 0.0f || measurements->current != 0.0f ||
...
void ina238_print_status(const ina238_device_t *dev) {
   ...
   if (dev->initialized) {
      ...
      printf("  ADC Range: %s\n",
             (dev->range == INA238_ADCRANGE_HIGH) ? "HIGH (±163.84mV)" : "LOW (±40.96mV)");
   }
   printf("\n");
}
