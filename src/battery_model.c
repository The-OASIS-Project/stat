...
static const tf_pair_t tf_liion[] = { { 25, 1.00f },
                                      { 0, 0.88f },
                                      { -10, 0.74f },
                                      { -20, 0.55f },
                                      { -30, 0.40f } };
static const tf_pair_t tf_lipo[] = { { 25, 1.00f },
                                     { 0, 0.90f },
                                     { -10, 0.78f },
                                     { -20, 0.60f },
                                     { -30, 0.45f } };
static const tf_pair_t tf_lifepo4[] = { { 25, 1.00f },
                                        { 0, 0.72f },
                                        { -10, 0.60f },
                                        { -20, 0.45f },
                                        { -30, 0.35f } };
static const tf_pair_t tf_nimh[] = { { 25, 1.00f },
                                     { 0, 0.85f },
                                     { -10, 0.70f },
                                     { -20, 0.55f },
                                     { -30, 0.40f } };
static const tf_pair_t tf_lead[] = { { 25, 1.00f },
                                     { 0, 0.46f },
                                     { -10, 0.40f },
                                     { -20, 0.30f },
                                     { -30, 0.20f } };
...
static float interp_tbl(const tf_pair_t *tbl, size_t n, float Tc) {
   if (Tc >= tbl[0].t)
      return tbl[0].f; /* ≥25 °C → 1.0 */
   if (Tc <= tbl[n - 1].t)
      return tbl[n - 1].f; /* clamp ≤ –30 °C */
...
static float battery_temp_capacity_factor(const battery_config_t *cfg, float temp_c) {
   switch (cfg->chemistry) {
      case BATT_CHEMISTRY_LIION:
         return interp_tbl(tf_liion, sizeof tf_liion / sizeof tf_liion[0], temp_c);
      case BATT_CHEMISTRY_LIPO:
         return interp_tbl(tf_lipo, sizeof tf_lipo / sizeof tf_lipo[0], temp_c);
      case BATT_CHEMISTRY_LIFEPO4:
         return interp_tbl(tf_lifepo4, sizeof tf_lifepo4 / sizeof tf_lifepo4[0], temp_c);
      case BATT_CHEMISTRY_NIMH:
         return interp_tbl(tf_nimh, sizeof tf_nimh / sizeof tf_nimh[0], temp_c);
      case BATT_CHEMISTRY_LEAD_ACID:
         return interp_tbl(tf_lead, sizeof tf_lead / sizeof tf_lead[0], temp_c);
...
float smooth_battery_runtime(float raw_time_min, float current_a, battery_source_t source_id) {
   ...
   float smoothed_time = (alpha * raw_time_min) + ((1.0f - alpha) * smoothed_times[source_id]);
...
battery_chemistry_t battery_chemistry_from_string(const char *chemistry_str) {
   ...
   if (strcasecmp(chemistry_str, "li-ion") == 0 || strcasecmp(chemistry_str, "liion") == 0) {
      return BATT_CHEMISTRY_LIION;
   }

   if (strcasecmp(chemistry_str, "lipo") == 0 || strcasecmp(chemistry_str, "li-po") == 0) {
      return BATT_CHEMISTRY_LIPO;
   }

   if (strcasecmp(chemistry_str, "lifepo4") == 0 || strcasecmp(chemistry_str, "life") == 0) {
      return BATT_CHEMISTRY_LIFEPO4;
   }

   if (strcasecmp(chemistry_str, "nimh") == 0 || strcasecmp(chemistry_str, "ni-mh") == 0) {
      return BATT_CHEMISTRY_NIMH;
   }

   if (strcasecmp(chemistry_str, "lead-acid") == 0 || strcasecmp(chemistry_str, "sla") == 0 ||
