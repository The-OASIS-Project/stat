...
int ina3221_detect_device(char *sysfs_path, size_t path_size) {
   ...
   while ((entry = readdir(dir)) != NULL) {
      ...
      /* Build device path */
      int len = snprintf(device_path, sizeof(device_path), "%s/%s", INA3221_SYSFS_BASE,
                         entry->d_name);
...
static int ina3221_init_channel(ina3221_device_t *dev, int channel) {
   ...
   OLOG_INFO("INA3221 Channel %d (%s): Enabled, Shunt=%.6f Ω", channel, ch->label,
             ch->shunt_resistor);
...
int ina3221_init(ina3221_device_t *dev) {
   ...
   OLOG_INFO("INA3221 initialized: %d active channels at %s", dev->num_active_channels,
             dev->sysfs_path);
...
int ina3221_read_channel(ina3221_device_t *dev, int channel, ina3221_channel_t *channel_data) {
   ...
   if (!dev || !dev->initialized || channel < 1 || channel > INA3221_MAX_CHANNELS ||
       !channel_data) {
      return -1;
   }
   ...
   if (dev->initialized) {
      ...
      for (int i = 0; i < INA3221_MAX_CHANNELS; i++) {
         ...
         printf("  Channel %d (%s): %s, Shunt=%.6f Ω\n", ch->channel, ch->label,
                ch->enabled ? "Enabled" : "Disabled", ch->shunt_resistor);
      }
   }
   printf("\n");
}
